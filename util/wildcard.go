...
// MatchMultipleSimple - finds whether the text matches/satisfies the pattern string.
// supports only '*' wildcard in the pattern.
// considers a file system path as a flat name space.
func MatchMultipleSimple(patterns []string, name string) bool {
	for  i := 0; i < len(patterns); i++ {
		if Match(patterns[i], name) {
			return true
		}
	}
	return false
